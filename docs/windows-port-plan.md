...
- `Project/`: `ImageCodec` (PNG/JPEG/WebP decode with EXIF orientation, PNG `pHYs` and JFIF density written by hand), `ProjectStore` (zip `.comp`, `com.compositor.windows.project` v1, atomic `File.Move`, full validation), `ProjectSnapshot` ↔ `CanvasDocument`.
