...
- Brush and Eraser vertical slice: basic round strokes, live preview, selection clipping, transformed-layer mapping, opacity, single-step undo and project round-trip are implemented. Next: colour picker, hardness/spacing, Shift straight lines, pressure, right-drag size/hardness, brush cursor overlay, and sparse tiled commits matching `BrushStroke.swift`.
